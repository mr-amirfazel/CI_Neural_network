...
class Model:
    ...
    def is_layer(self, layer):
        ...
        """
        # TODO: Implement check if the layer is a layer ----- done
        check = type(layer) == FC or type(layer) == MaxPool2D or type(layer) == Conv2D
        return check
    ...
    def is_activation(self, layer):
        ...
        """
        # TODO: Implement check if the layer is an activation  ----- done
        check = type(layer) == Sigmoid or type(layer) == ReLU or type(layer) == Tanh or type(layer) == LinearActivation
        return check
    ...
    def backward(self, dAL, tmp, x):
        ...
        dA = dAL
        grads = {}
        # TODO: Implement backward pass through the model   ----- done
        # NOTICE: we have a pattern of layers and activations
        # for from the end to the beginning of the tmp list
        for l in range(len(tmp) - 1, 0, -2):
            if l > 2:
                Z, A = tmp[l - 1], tmp[l - 2]
            else:
                Z, A = tmp[l - 1], x
    ...
    def update(self, grads):
        ...
        for l in self.layers_names:
            if self.is_layer(self.model[l]) and not type(self.model[
                                                             l]) == MaxPool2D:  # hint check if the layer is a layer and also is not a maxpooling layer
                self.model[l].update_parameters(self.optimizer, grads[l])
    ...
    def one_epoch(self, x, y, batch_size):
        ...
        returns:
            loss
        """
        # TODO: Implement one epoch of training ----- done
        tmp = self.forward(x)
        AL = tmp[len(self.layers_names) - 1]
    ...
    def batch(self, X, y, batch_size, index, order):
        ...
        returns:
            bx, by: batch of data
        """
        # TODO: Implement batch ----- done
        # last_index = min(index * batch_size, len(order))   # hint last index of the batch check for the last batch
    ...
    def compute_loss(self, X, y, batch_size):
        ...
        returns:
            loss
        """
        # TODO: Implement compute loss ----- done
        ...
        for b in range(m // batch_size):
            ...
            tmp = self.forward(bx)
            AL = tmp[-1]
            cost += self.criterion.commpute_cost(AL, y)(m // batch_size)
        return cost
    ...
    def train(self, X, y, epochs, val=None, batch_size=3, shuffling=False, verbose=1, save_after=None):
        ...
        for e in tqdm(range(1, epochs + 1)):
            ...
            for b in range(m // batch_size):
                ...
                cost += self.one_epoch(bx, by, batch_size) / (m // batch_size)
    ...
    def predict(self, X):
        ...
        returns:
            predictions
        """
        # TODO: Implement prediction ----- done
