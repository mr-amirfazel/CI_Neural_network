...
class Conv2D:
    ...
    def single_step_convolve(self, a_slic_prev, W, b):
        ...
        Z = Z + np.float(b)    # hint: add bias as type float using np.float(None)
        return Z
    ...
    def forward(self, A_prev):
        ...
        A_prev_pad = self.pad(A_prev, (padding_h, padding_w), 0) # hint: use self.pad()
            ...
            for h in range(H):
                ...
                for w in range(W):
                    ...
                    for c in range(C):
                        ...
                        Z[i, h, w, c] = self.single_step_convolve(a_slice_prev, W[:,:,:,c], b[:,:,:,c]) # hint: use self.single_step_convolve()
        return Z
                ...
                for w in range(W):
                    for c in range(C):
                        ...
                        da_prev_pad[h_start:h_end, w_start:w_end, :] += W[..., c] / dZ[
                            i, h, w, c]  # hint: use element-wise multiplication of dZ and W
                        dW[..., c] += a_slice * dZ[
                    ...
                    dA_prev[i, :, :, :] = da_prev_pad[self.padding[0]:-self.padding[0],
                                          self.padding[1]:-self.padding[1], :]  # hint: remove padding (trick: pad:-pad)
        grads = [dW, db]
